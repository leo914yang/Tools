...
# model = torch.hub.load('ultralytics/yolov5', 'custom',
#     path='/Users/leo/Downloads/yolov5l/weights/best.pt', force_reload=True)

# model = torch.hub.load('./yolov7','custom',
#     path_or_model='/Users/leo/git_workspace/yolov7/weights/best.pt',source='local',force_reload=True)
...
while True:
    ...
    if cap.tracking:
        ...
        # max_index = 0
        # if len(df.values):
        #     for index, i in enumerate(df['confidence'].values):
        #         if max_conf < i:
        #             max_conf = i
        #             max_index = index
        #     cv2.putText(image, df['name'][max_index], (int(xmin[index])+100, int(ymin[index])), cv2.FONT_HERSHEY_SIMPLEX,
        #             1, (255, 0, 0), 1, cv2.LINE_AA)
        #     conf = df['confidence'][max_index]
        #     cv2.putText(image, f'{conf:.2f}', (int(xmin[index])+400, int(ymin[index])), cv2.FONT_HERSHEY_SIMPLEX,
        #             1, (255, 0, 0), 1, cv2.LINE_AA)
        ...
        for index, i in enumerate(order):
            ...
            cv2.putText(image, str(index), (int(xmin[i]), int(ymin[i])), cv2.FONT_HERSHEY_SIMPLEX,
                1, (0, 0, 255), 1, cv2.LINE_AA)
            cv2.putText(image, df['name'][index], (int(xmin[i])+100, int(ymin[i])), cv2.FONT_HERSHEY_SIMPLEX,
                    1, (0, 0, 255), 1, cv2.LINE_AA)
            conf = df['confidence'][index]
            cv2.putText(image, f'{conf:.2f}', (int(xmin[i])+400, int(ymin[i])), cv2.FONT_HERSHEY_SIMPLEX,
                    1, (0, 0, 255), 1, cv2.LINE_AA)
