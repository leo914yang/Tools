...
def Move_Rename():
    ...
    for i in range(img_num):
        ...
        new_img_name = str(c) + '_' + img_name

        lab_name = ''.join(lab_path[i].split('labels/')[1].split('.jpg'))
        new_lab_name = str(c) + '_' + lab_name

        if i <= img_proportion:
            shutil.move(img_path[i], str(opt.d1) + 'images/' + new_img_name)
            shutil.move(lab_path[i], str(opt.d1) + 'labels/' + new_lab_name)
        else:
            shutil.move(img_path[i], str(opt.d2) + 'images/' + new_img_name)
            shutil.move(lab_path[i], str(opt.d2) + 'labels/' + new_lab_name)
